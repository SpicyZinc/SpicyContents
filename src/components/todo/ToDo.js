...
class ToDo extends Component {
	...
	toggleTask(e) {
		...
		axios.post(`http://localhost:3000/todo/tasks/${taskToUpdate.id}/edit`, taskToUpdate)
			.then((res) => {
	...
	loadFromServer() {
		axios.get('http://localhost:3000/todo/tasks')
			.then(res => {
	...
	handleSubmit(e, task) {
		...
		axios.post("http://localhost:3000/todo/tasks/create", newTask)
			.then((res) => {
				this.setState({
